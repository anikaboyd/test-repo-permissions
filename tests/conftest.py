...
from test_project_anika_b import TestProjectAnikaB, DefaultAioHttpClient, AsyncTestProjectAnikaB
from test_project_anika_b._utils import is_dict
...
logging.getLogger("test_project_anika_b").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[TestProjectAnikaB]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with TestProjectAnikaB(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncTestProjectAnikaB]:
